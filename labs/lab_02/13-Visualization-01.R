...
barplot(scores,
        ...
        col = c(1, 2, 3, 4, 5),
        ylim = c(0, 10))
text(x = 1:5 *1.2 - 0.5,y = scores + 0.5, labels = scores)

barplot(scores,
        names.arg = "Điểm thi của bạn",
        col = c(1, 2, 3, 4, 5),
        xlim = c(0, 10),
        horiz = TRUE)
...
#    - Loài nào có petal dài nhất? Loài virginica
#    - Loài nào có độ biến thiên lớn nhất?Loài virginica
#    - Có outliers không? Ở loài nào? Có ở loài viriginica

boxplot(iris$Sepal.Length ~ iris$Species,
        col = c(1, 2, 3),
        main = "So Sánh độ dài giữa 3 loài hoa",
        xlab = "Loài hoa",
        ylab = "Độ dài")
...
abline(lm(hp ~ mpg, mtcars), col = "red", lwd = 2)

unique_cyl <- unique(mtcars$cyl)
legend("topright",
       legend = factor(unique_cyl), 
       col = 1:3,
       lwd = 2)

# Nhận xét: hp tỉ lệ nghịch với mpg
...
# Tạo ma trận: Mỗi hàng là một sản phẩm, mỗi cột là một quý
data_matrix <- matrix(c(Q1, Q2, Q3, Q4), nrow = 4, byrow = FALSE)
rownames(data_matrix) <- products

colnames(data_matrix) <- c("Q1", "Q2", "Q3", "Q4")
data_matrix
...
barplot(data_matrix, beside = TRUE, col = terrain.colors(4),
        main = "Doanh thu theo Quý", xlab = "Quý", ylab = "Doanh thu",
        legend.text = products, args.legend = list(x = "topleft", bty = "n", cex = 0.7))

# 2. Line Plot tất cả sản phẩm
matplot(t(data_matrix), type = "b", pch = 16, lty = 1, col = 1:4,
        xaxt = "n", main = "Xu hướng Sản phẩm", xlab = "Quý", ylab = "Doanh thu")
legend("topleft", legend = products, col = 1:4, pch = 16, cex = 0.7, bty = "n")

# 3. Pie Chart tổng doanh thu mỗi Quý
total_q <- colSums(data_matrix)
pie(total_q, labels = paste(colnames(data_matrix), "-", total_q), 
    col = rainbow(4), main = "Tỷ trọng Doanh thu Quý")

# 4. Bar Chart tổng doanh thu mỗi Sản phẩm
total_sp <- rowSums(data_matrix)
barplot(total_sp, names.arg = products, col = "skyblue",
        main = "Tổng Doanh thu SP", xlab = "Sản phẩm", ylab = "Doanh thu")
